...
namespace ProceduralCreature.Morphology.Extraction
{
    ...
    /// per-cube loops. The extractor owns shared grid-edge/corner identity and
    /// winding; CubeContourResolver owns only per-cube contour topology.
    ///
    /// Vertex welding uses direct integer ownership tables rather than a hash map.
    /// Each canonical X/Y/Z grid edge has one stable slot, and exact surface
    /// corners use a separate corner table. An owner is stored as vertexIndex + 1,
    /// leaving zero as the allocation-free "unclaimed" sentinel.
    ///
    /// Winding uses the cached DensityGrid gradient; no SDF evaluation occurs during
    /// extraction. The output order is deterministic because cells, contour loops,
    /// and each ownership table are traversed in fixed integer order.
    /// </summary>
    public static partial class MarchingCubesExtractor
    {
        ...
        private static MeshExtractionResult ExtractCachedGrid(DensityGrid grid, bool collectTimings)
        {
            ...
            for (int i = 0; i < activeCells.Length; i++)
            {
                ...
                foreach (List<CubeContourResolver.LoopVertex> loop in loops)
                {
                    ...
                    EmitLoop(
                        grid, loop, cx, cy, cz, ownership, result, loopIndices,
            ...
            MeshExtractionResult result)
        {
            ...
            float sampleA = GenerationTolerances.NormalizeSurfaceDensity(grid.GetSample(gridA.x, gridA.y, gridA.z));
            if (sampleA == 0f)
            {
                return ownership.ResolveCorner(gridA.x, gridA.y, gridA.z, positionA, result);
            }

            float sampleB = GenerationTolerances.NormalizeSurfaceDensity(grid.GetSample(gridB.x, gridB.y, gridB.z));
            if (sampleB == 0f)
            {
        ...
        {
            private readonly int _cornersX;
            private readonly int _cornersY;
            private readonly int _cellsX;
            private readonly int _cellsY;
            private readonly int _cellsZ;
            private readonly int _xEdgeCount;
            private readonly int _yEdgeCount;
            private readonly int[] _cornerOwners;
            private readonly int[] _edgeOwners;

            public GridVertexOwnership(DensityGrid grid)
            {
                _cornersX = checked(grid.CellsX + 1);
                _cornersY = checked(grid.CellsY + 1);
                _cellsX = grid.CellsX;
                _cellsY = grid.CellsY;
                _cellsZ = grid.CellsZ;

                long cornerCount = (long)_cornersX * _cornersY * checked(_cellsZ + 1);
                long xEdgeCount = (long)_cellsX * _cornersY * checked(_cellsZ + 1);
                long yEdgeCount = (long)_cornersX * _cellsY * checked(_cellsZ + 1);
                long zEdgeCount = (long)_cornersX * _cornersY * _cellsZ;
                long edgeCount = xEdgeCount + yEdgeCount + zEdgeCount;
                if (cornerCount > int.MaxValue || edgeCount > int.MaxValue)
                {
                    throw new DomainException("Grid vertex ownership table exceeds addressable array size.");
                }

                _xEdgeCount = (int)xEdgeCount;
                _yEdgeCount = (int)yEdgeCount;
                _cornerOwners = new int[(int)cornerCount];
                _edgeOwners = new int[(int)edgeCount];
            }
            ...
            public int ResolveEdge(int axis, int x, int y, int z, Vector3 position, MeshExtractionResult result)
            {
                ...
                {
                    case 0:
                        localIndex = ((z * _cornersY) + y) * _cellsX + x;
                        offset = 0;
                        break;
                    case 1:
                        localIndex = ((z * _cellsY) + y) * _cornersX + x;
                        offset = _xEdgeCount;
                        break;
                    case 2:
                        localIndex = ((z * _cornersY) + y) * _cornersX + x;
                        offset = _xEdgeCount + _yEdgeCount;
                        break;
                    default:
                        throw new DomainException("edge axis must be X (0), Y (1), or Z (2).");
                }
