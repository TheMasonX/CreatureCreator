...
namespace ProceduralCreature.Tests.Editor
{
    ...
    public class NumericValidityTests
    {
        ...
        [Test]
        public void IsFinite_Matrix4x4_FiniteValues_ReturnsTrue()
        {
            Assert.IsTrue(NumericValidity.IsFinite(Matrix4x4.identity));
        }

        [Test]
        public void IsFinite_Matrix4x4_NonFiniteElement_ReturnsFalse()
        {
            Matrix4x4 matrix = Matrix4x4.identity;
            matrix.m23 = float.NaN;
            Assert.IsFalse(NumericValidity.IsFinite(matrix));

            matrix = Matrix4x4.identity;
            matrix.m10 = float.PositiveInfinity;
            Assert.IsFalse(NumericValidity.IsFinite(matrix));
        }
